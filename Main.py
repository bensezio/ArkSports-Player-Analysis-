...
def batch_prediction(df):
    ...
    if st.button("Run Predictions"):
        ...
        def min_max_scale(group):
            min_pred = group['Predicted_Performance'].min()
            max_pred = group['Predicted_Performance'].max()
            if max_pred > min_pred:
                group['Predicted_Performance_Percent'] = (
                    (group['Predicted_Performance'] - min_pred) / (max_pred - min_pred)
                ) * 100
            else:
                group['Predicted_Performance_Percent'] = 50  # Default if all predictions are equal
            return group
        
        df = df.groupby('model_key').apply(min_max_scale)
        
        # Drop temporary column
        df = df.drop('model_key', axis=1)
        
# Sorting option (MOVE THIS OUTSIDE THE BUTTON CLICK)
...
# Display results with sorting applied after predictions run
display_columns = ['Player', 'Position_Cleaned', 'Predicted_Performance', 'Predicted_Performance_Percent']
if all(col in df.columns for col in display_columns):
    df_sorted = df.sort_values(by="Predicted_Performance", ascending=ascending)
    st.write(df_sorted[display_columns])
